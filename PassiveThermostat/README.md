# PassiveThermostat
### Update
I am in the process of revising this code to add calculation for wet bulb temperature, which does not require an actual wet bulb thermometer, and notification for when it exceeds the currently standard theoretical danger threshold of ~30c, as discovered and published [here](https://journals.physiology.org/doi/abs/10.1152/japplphysiol.00738.2021). I'm also taking this opportunity to refactor the codebase into a format which is more aligned with "best practice" in terms of keeping things simple and well defined, as well as altering the logic/premise underlying this project to be more useful. Additionally, I've come to the conclusion that I should move the details about configuring raspberry pi out of the sub-folder readmes and into the PyOT readme. This will reduce redundancy and allow for a single reference for the device I will likely be using for most, if not all, of these projects, as well as cleaning up the readme for each project to be more specific to that use case.

I should also note that while below I state this uses apparent temperature, that was never implemented (though I may have mistakenly thought heat index is equivalent to apparent temperature, this is not technically true), though is still something I would like to work towards. Of particular interest is developing an opensource version of the proprietary algorithms/formulas used by The Weather Company, etc., and which may take into consideration factors like UV index, cloud coverage, etc. However, I will first need to find a way to get wind speed from either an API or sensor (perhaps something like [this](https://projects.raspberrypi.org/en/projects/build-your-own-weather-station/5), or maybe a DIY version) in order to finally calculate apparent temperature.

Other metrics of potential interest include the wet bulb globe temperature, which differs from wet bulb temperature in that it estimates the effects of solar radiation by using a weighted average of a thermometer in a black globe with the wet bulb temperature and "normal" temperature, and an alternative form of wet bulb globe temperature which is identical except that it does not account for solar radiation (no black globe). Both of these are measures of a type of "apparent" temperature. That is to say, they differ from the basic wet bulb temperature in that they are more closely aligned with how a person experiencing the conditions would describe them. [This](https://www.omnicalculator.com/physics/wet-bulb#how-to-calculate-the-wet-bulb-temperature) website provided the equation for the basic wet bulb calculation I've already implemented, and includes equations for wet bulb globe temperature with and without solar radiation.

I may also eventually add support for other temperature/humidity sensor types, as it appears the DHT11 is no longer sold by Adafruit (though that doesn't mean you can't get them somewhere else). This may necessitate further revision to align the code more closely with the circuit python implementation detailed [here](https://learn.adafruit.com/dht/dht-circuitpython-code), but that's fairly low on my list of priorities. Ultimately, the end-all-be-all for this project would be an "environmentally aware" domicile, which can open and close its own windows, turn on fans, trigger heating/cooling if/as needed, etc. according to the logic we provide, in order to minimize the cost to the user in terms of electricity/gas used to heat/cool the space. This is a very long-term possibility, but a possibility nonetheless. However, I'm sure, once the project approaches that point, and probably before, the text-based interface will likely be seen as lacking, and I will need to look into providing a GUI/mobile app, integrating this with Matter, which you can learn more about [here](https://www.wired.com/story/what-is-matter/), etc.
...
Connect the power pin to either a 3.3 or 5v power pin, the ground pin to ground, and the data pin to one of the Pi's GPIO data pins. Be sure to note which data pin you've chosen, as you will need to alter the script to reflect that. In the script, I have specified GPIO pin four, which is physical pin 7, for all sensor instances in the script. This may be a bit confusing if you've never worked with electronics before, so I encourage you to review the Raspberry Pi's GPIO pinout documentation, which can be [found here](https://www.raspberrypi.com/documentation/computers/os.html#gpio-and-the-40-pin-header). In sum, it's important to realize that the physical pin numbering is independent of the GPIO number, since there is no need to count or refer to pins that do not handle data. After all, you'll likely never need to ping the voltage line for information.
